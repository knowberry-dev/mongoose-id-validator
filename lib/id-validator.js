...
IdValidator.prototype.validateSchema = function (
    ...
    schema.eachPath(function (path, schemaType) {
        ...
        if (schemaType.options && schemaType.options.ignoreValidateRef) {
            return;
        }
        ...
        if (schemaType.options && schemaType.options.ref) {
            ...
            refModelPath = schemaType.options.validateRefPath || schemaType.options.refPath;
            if (schemaType.options.refConditions) {
                conditions = schemaType.options.refConditions;
            }
                    ...
                    validator: function (value) {
                        return new Promise(function (resolve, reject) {
                            ...
                            if (!(self instanceof IdValidator) || self.enabled) {
                                ...
                                return validateFunction(this, connection, message, localRefModelName,
...
function executeQuery(query, conditions, validateValues, resolve, reject, message) {
    ...
    query
        ...
        .exec(function (err, documents) {
            ...
            const difference = validateValues.filter(val => !ids.includes(String(val)));

            return difference.length === 0 ? resolve(true) : reject({
                message,
                detail: { values: difference }
            });
        });
}

function validateId(
    doc, connection, message, refModelName, value, conditions, resolve, reject) {
    if (value == null) {
        resolve(true);
        return;
    }
    var refModel = connection.model(refModelName);
    var query = refModel.find({ _id: value });
    var session = doc.$session && doc.$session();
    if (session) {
        query.session(session);
    }
    executeQuery(query, conditions, [value], resolve, reject, message);
}

function validateIdArray(
    doc, connection, message, refModelName, values, conditions, resolve, reject,
    allowDuplicates) {
    ...
    var refModel = connection.model(refModelName);
    var query = refModel.where('_id')['in'](checkValues);
    var session = doc.$session && doc.$session();
    if (session) {
        query.session(session);
    }

    executeQuery(query, conditions, checkValues, resolve, reject, message);
}
